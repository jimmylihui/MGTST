...
class FFT1D_block(nn.Module):
    ...
    def fft_1D(self, x):
        if self.num:
            ...
            x_ft[:,100:,:]=0
            x = torch.fft.irfft(x_ft,dim=1, n=self.num)
            x = x.real 
        else:
            ...
            x = torch.fft.irfft(x_ft,dim=1, n=self.out_channels)
            x = x.real 
        return x
...
class attention_block(nn.Module):
    def __init__(self, modes1,modes2, num=None):
        ...
        self.scale = (1 / (modes1*modes2))
        self.weights_FFT = nn.Parameter(self.scale * torch.rand(modes1,modes2, dtype=torch.cfloat))
        self.weights_FFT_2 =nn.Parameter(self.scale * torch.rand(720,720, dtype=torch.cfloat))
    def fft_1D(self, x):
        if self.num:
            
            x_ft = torch.fft.fft(x,dim=1,n=self.num)
