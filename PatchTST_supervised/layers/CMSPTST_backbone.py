...
# Cell
import math
class CMSPTST_backbone(nn.Module):
    def __init__(self, configs,scale,gate,c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
        ...
        self.merge_layer=SegMerging(d_model,win_size=2)
        ...
        #     # Patching
        #     patch_len_stage = patch_len*(pow(2,i))

        #     stride_stage = stride*(pow(2,i))
        ...
        #     patch_num=math.ceil(patch_num/2)

            
        self.backbone=TSTiEncoder(1, patch_num=target_window, patch_len=c_in, max_seq_len=max_seq_len,
                                n_layers=n_layers, d_model=c_in, n_heads=1, d_k=d_k, d_v=d_v, d_ff=d_ff,
                                attn_dropout=attn_dropout, dropout=dropout, act=act, key_padding_mask=key_padding_mask, padding_var=padding_var,
                                attn_mask=attn_mask, res_attention=res_attention, pre_norm=pre_norm, store_attn=store_attn,
                                pe=pe, learn_pe=learn_pe, verbose=verbose, **kwargs)
    ...
    def forward(self, x,*args):                                                                   # z: [bs x nvars x seq_len]
        # norm
